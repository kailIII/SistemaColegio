...
Public Class frmCargaAcademicaTraslado
...
#End Region
  ...
  Public Sub Seleccionar(ByVal grado As String)
    variable = grado
      ...
      If sw2 = 1 Or swClick = 1 Then
        ...
        For i As Integer = 0 To Me.dgAlumnos.RowCount - 1
          If CBool(Me.dgAlumnos.Rows(i).Cells(2).EditedFormattedValue) <> arrayAlumnos(i) Then
            ...
            ldc = objD.CodigosDocenteCarga(New Grupo(New Matricula(CType(Me.dgAlumnos.Rows(i).DataBoundItem, Matricula).Numero), New DocenteCarga(0, "", New Docente(), New CargaAcademica(0, New Curso(CType(Me.dgCursos.CurrentRow.DataBoundItem, Curso).Codigo), New Seccion()), New PeriodoAcademico())))

            If CBool(Me.dgAlumnos.Rows(i).Cells(2).EditedFormattedValue) Then
              'primero eliminar
              If ldc.Count > 0 Then
                For Each dc In ldc
                  obj.Eliminar(New Grupo(New Matricula(CType(Me.dgAlumnos.Rows(i).DataBoundItem, Matricula).Numero, New Alumno(), 0), New DocenteCarga(dc.Codigo)))
                Next
              End If

              'luego registrar
              pos = Me.dgGrupos.Rows.IndexOf(Me.dgGrupos.CurrentRow)
              ldc = objD.CodDocCarNombreGrupo(New DocenteCarga(0, Me.dgGrupos.Rows(pos).Cells(1).EditedFormattedValue.ToString, New Docente(), New CargaAcademica(vectorCarga(pos)), New PeriodoAcademico()))
              For Each dc In ldc
                obj.Registrar(New Grupo(New Matricula(CType(Me.dgAlumnos.Rows(i).DataBoundItem, Matricula).Numero, New Alumno(), 0), New DocenteCarga(dc.Codigo)))
              Next
            Else
              'solo eliminar
              If ldc.Count > 0 Then
  ...
  Public Sub ListarAlumnosAula(ByVal letra As String)
    ...
    lm = obj.ListarAlumnosSeccion(New CargaAcademica(0, New Curso(CType(Me.dgCursos.CurrentRow.DataBoundItem, Curso).Codigo), New Seccion(CType(Me.dgCursos.CurrentRow.DataBoundItem, Curso).Grado.Codigo, moAnioActual.anio, letra)))
    If lm.Count > 0 Then
  ...
  Public Sub AgregarEstados()
    ...
    If pos > -1 Then
      cur = CType(Me.dgCursos.CurrentRow.DataBoundItem, Curso)
      sec = seccion 'CType(Me.cboGrado.SelectedItem, Seccion)
      ldc = obj.ListarAlumnosGrupo2(New DocenteCarga(vectorDocCar(pos), Me.dgGrupos.Rows(pos).Cells(1).EditedFormattedValue.ToString, New Docente(), _
                                              New CargaAcademica(0, New Curso(cur.Codigo), _
                                              New Seccion(moAnioActual, New Grado(sec.Grado.Codigo), sec.Letra)), _
                                              New PeriodoAcademico(vectorPer(pos))), k)
    ...
    Try
      If Me.dgGrupos.RowCount > 1 Then
        ...
        If MessageBox.Show("En verdad desea quitar el grupo", Me.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Information) = Windows.Forms.DialogResult.Yes Then
          obj = New RNGrupo
          codigo = obj.CodigoCarga(New DocenteCarga(0, nombre, New Docente(), New CargaAcademica(0, New Curso(CType(Me.dgCursos.CurrentRow.DataBoundItem, Curso).Codigo), New Seccion(CType(Me.dgCursos.CurrentRow.DataBoundItem, Curso).Grado.Codigo, moAnioActual.anio, s.Letra)), New PeriodoAcademico()))
          If codigo > 0 Then

            objD = New RNDocenteCarga

            ldc = objD.CodigosDocenteCarga(New DocenteCarga(0, Me.dgGrupos.Rows(pos).Cells(1).EditedFormattedValue.ToString, _
                                                    New Docente(), New CargaAcademica(codigo), New PeriodoAcademico()))
            For Each dc As DocenteCarga In ldc
              obj.EliminarGrupo(New Grupo(New Matricula(), New DocenteCarga(dc.Codigo)))
            Next
          End If
