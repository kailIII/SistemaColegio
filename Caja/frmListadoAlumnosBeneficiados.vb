...
Public Class frmListadoAlumnosBeneficiados
  ...
  Private Sub btnListar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnListar.Click
    ...
    If Me.cboAnio.SelectedIndex > -1 Then
      rn = New RNBeneficio
      If Me.cboFiltro.SelectedIndex = 0 Then
        Me.dtAlumnos = rn.ListarAlumnosBeneficiadosAnio(DirectCast(Me.cboAnio.SelectedItem, AnioLectivo))
      Else
        Me.dtAlumnos = rn.ListarAlumnosBeneficiadosAnioFecha(DirectCast(Me.cboAnio.SelectedItem, AnioLectivo), Me.dtpInicio.Value, Me.dtpFin.Value)
      End If
      rn = Nothing
    ...
    Try
      If Me.dtAlumnos IsNot Nothing AndAlso Me.dtAlumnos.Rows.Count > 0 Then
        ...
        If Me.cboFiltro.SelectedIndex = 1 Then
          Me.mDialogoGuardar.FileName = "Listado de alumnos beneficiados " & Me.cboAnio.Text & " - " & Me.dtpInicio.Value.ToString("dd-MM-yyyy")
        Else
          Me.mDialogoGuardar.FileName = "Listado de alumnos beneficiados " & Me.cboAnio.Text
        End If
        m_excel = modExcel.CrearExcel()
        Libro = modExcel.AbrirPlantilla(m_excel, Me.mDialogoGuardar, AppDomain.CurrentDomain.BaseDirectory & "\Plantillas\ListadoAlumnosBeneficiados.xls")
        If Libro IsNot Nothing Then
          Hoja = DirectCast(Libro.Worksheets.Item("Datos"), Worksheet)
          If Me.cboFiltro.SelectedIndex = 1 Then
            Hoja.Range("B5").Value = "LISTADO DE BENEFICIOS REGISTRADOS EL " & Me.dtpInicio.Value.ToString("dd-MM-yyyy")
          End If
          Hoja.Range("C7").Value = Me.cboAnio.Text

          Me.ExportarReporte(Hoja.Range(CELDAINICIO).Offset(1, 0))

          modExcel.BordesTodo(Hoja.Range(Hoja.Range(CELDAINICIO).Offset(1, 0), Hoja.Range(CELDAINICIO).Offset(Me.dtAlumnos.Rows.Count, 11)))
  ...
  Private Sub ExportarReporte(ByVal Celda As Range)
    ...
    For Each dr As DataRow In Me.dtAlumnos.Rows
      ...
      Celda.Offset(0, 4).Value = dr.Item("Fecha")
      Celda.Offset(0, 5).Value = dr.Item("Motivo")
      Celda.Offset(0, 6).Value = dr.Item("Descuento")
      Celda.Offset(0, 7).Value = dr.Item("Monto")
      Celda.Offset(0, 8).Value = dr.Item("Padre")
      Celda.Offset(0, 9).Value = dr.Item("Madre")
      Celda.Offset(0, 10).Value = dr.Item("Direccion")
      Celda.Offset(0, 11).Value = dr.Item("Telefono")
